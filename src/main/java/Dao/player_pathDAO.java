...
public class player_pathDAO {
    public boolean add(tPlayer_path player) throws SQLException {
        Connection conn = DBConnection.getConn();

        String checkQuery = "SELECT * FROM player_path WHERE player_name = ?";
...
//            tp.getCurrentRoom();
            String upsql = "update player_path set room_path=? where player_name=?";
            try {
                PreparedStatement ps = conn.prepareStatement(upsql);
                ps.setInt(1, player.getRoom_path());
...
//            关闭数据库
            ...
            String sql = "insert into player_path values(?,?)";
            try {
                PreparedStatement ps = conn.prepareStatement(sql);
                ps.setString(1, player.getPlayer_name());
                ps.setInt(2, player.getRoom_path());
...
//    public boolean update(tPlayer_path player){
//        Connection conn =DBConnection.getConn();
//        String sql = "update player_path set room_path=? where player_name=?";
//
//        try {
//            PreparedStatement ps = conn.prepareStatement(sql);
//            ps.setString(1, player.getRoom_path());
//            ps.setString(2, player.getPlayer_name());
//
//            boolean resule = ps.executeUpdate() > 0;
//
...
//    }
    ...
    public boolean delete(String player_name){
        Connection conn =DBConnection.getConn();
        String sql = "delete from player_path where player_name=?";
...
//            关闭数据库
    ...
    public tPlayer_path query(String player_name) {
        Connection conn = DBConnection.getConn();
        //2、预编译sql执行
        String sql = "select * from player_path where player_name=?";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setString(1, player_name);
    ...
    public List<tPlayer_path> getListAll(){
        Connection conn = DBConnection.getConn();
        //2、预编译sql执行
        String sql = "select * from player_path";
        try {
            PreparedStatement ps = conn.prepareStatement(sql);
